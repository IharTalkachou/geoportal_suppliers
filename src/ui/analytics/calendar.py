...
def render_calendar_tab():
    ...
    # Для блока "В процессе" мы всегда берем только 'В работе' и 'Ожидание'
    ...
    for i in range(8):  # Сегодня + 7 дней
        ...
        # 🟢 ИСПРАВЛЕНИЕ: Теперь берем из display_cal_df (уже отфильтрованного по статусу)
        starts = display_cal_df[display_cal_df['planned_start'].dt.normalize() == current_day_ts]
        ends = display_cal_df[display_cal_df['planned_end'].dt.normalize() == current_day_ts]
        ...
        if not starts.empty or not ends.empty or not in_progress.empty:
            ...
            with st.expander(f"{header_prefix}, {day_str} — событий: {evt_count}, в работе: {len(in_progress)}", expanded=(i==0)):
                ...
                # Вспомогательная функция формирования строки
                def format_agenda_row(r, show_overdue_logic=True):
                    ...
                    overdue_info = f" | **План. конец - {r['planned_end'].strftime('%d.%m.%Y')}**" if is_overdue else ""
                ...
                # --- 1. Ключевые события дня ---
                if not starts.empty or not ends.empty:
                    st.markdown("**🎯 Ключевые события дня:**")
                    for _, r in starts.iterrows():
                        # Для ключевых событий тоже добавляем проверку просрочки, если они в работе
                        is_overdue = (r['status'] != 'Выполнено' and pd.notna(r['planned_end']) and r['planned_end'].normalize() < today_ts)
                        prefix = "⚡ " if is_overdue else "🚀 "
                        st.write(f"{prefix}**Старт**: {r['project_name']} | {r['stage_name']}")
                    
                    for _, r in ends.iterrows():
                        is_overdue = (r['status'] != 'Выполнено' and r['planned_end'].normalize() < today_ts)
                        prefix = "⚡ " if is_overdue else "🏁 "
                        st.write(f"{prefix}**Дедлайн**: {r['project_name']} | {r['stage_name']}")
